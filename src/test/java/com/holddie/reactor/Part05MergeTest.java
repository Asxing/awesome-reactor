...
import com.holddie.reactor.domain.User;
import com.holddie.reactor.repository.ReactiveRepository;
import com.holddie.reactor.repository.ReactiveUserRepository;
...
public class Part05MergeTest {
    ...
    public void mergeWithInterleave() {
        Flux<User> flux =
                workshop.mergeFluxWithInterleave(
                        repositoryWithDelay.findAll(), repository.findAll());
    ...
    public void mergeWithNoInterleave() {
        Flux<User> flux =
                workshop.mergeFluxWithNoInterleave(
                        repositoryWithDelay.findAll(), repository.findAll());
        StepVerifier.create(flux)
