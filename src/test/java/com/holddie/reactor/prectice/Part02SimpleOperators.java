...
public class Part02SimpleOperators implements BaseTestFlux {
     ...
     * increment of time before emitting the events which are thus shifted by the specified time
     * amount.
     *
     * <p>The delay operator uses a Scheduler {@see Part07Schedulers} by default, which actually
     * means it's running the operators and the subscribe operations on a different thread, which
     * means the test method will terminate before we see the text from the log.
     *
     * <p>To prevent this we use the CountDownLatch that we decrement on onComplete
     */
    @Test
    ...
    public void intervalOperator() {
        ...
        Flux<Long> flux = Flux.interval(Duration.of(1, ChronoUnit.SECONDS)).take(5);

        subscribeWithLogWaiting(flux);
    }

    /**
     * scan operator - takes an initial value and a function(accumulator, currentValue). It goes
     * through the events sequence and combines the current event value with the previous
     * result(accumulator) emitting downstream the The initial value is used for the first event
     */
    @Test
    ...
    /**
     * collect operator acts similar to the reduce() operator, but while the reduce() operator uses
     * a reduce function which returns a value, the collect() operator takes a container supplie and
     * a function which doesn't return anything(a consumer). The mutable container is passed for
     * every event and thus you get a chance to modify it in this collect consumer function
     */
    @Test
    public void collectOperator() {
        Mono<List<Integer>> numbers =
                Flux.just(3, 5, -2, 9)
                        .collect(
                                ArrayList::new,
                                (container, value) -> {
