...
public class Part09BackpressureHandling implements BaseTestFlux {
    ...
    public void fluxWithCreateHasBackpressureSupport() {
        ...
        Flux<Integer> flux =
                createFlux(10, FluxSink.OverflowStrategy.LATEST)
                        .log()
                        .publishOn(Schedulers.newElastic("elast"), 5);

        flux.subscribe(
                logNextAndSlowByMillis(50), // simulate a slow subscriber
    ...
    /**
     * Not only a slow subscriber triggers backpressure, but also a slow operator that would slow
     * down the handling of events and new request calls for new items
     */
    @Test
    public void backpressureStrategyTriggeredBySlowOperator() {
        Flux<String> flux =
                createFlux(20, FluxSink.OverflowStrategy.IGNORE)
    ...
    public void fluxWithCustomLogicOnBackpressureBuffer() {
        ...
        Flux<Integer> flux =
                createFlux(15, FluxSink.OverflowStrategy.ERROR)
    ...
    public void zipOperatorHasALimit() {
        ...
        Flux<String> zippedFlux = Flux.zip(fast, slowStream, (val1, val2) -> val1 + " " + val2);
    ...
    private class CustomFlux extends Flux<Integer> {
        ...
        class CustomRangeSubscription implements Subscription {
            ...
            CustomRangeSubscription(
                    int startFrom, int count, Subscriber<? super Integer> actualSubscriber) {
                this.count = count;
                this.startFrom = startFrom;
    ...
    private <T> void subscribeWithSlowSubscriber(Flux<T> flux, CountDownLatch latch) {
        flux.subscribe(
                logNextAndSlowByMillis(200), logErrorConsumer(latch), logCompleteMethod(latch));
    }
}
