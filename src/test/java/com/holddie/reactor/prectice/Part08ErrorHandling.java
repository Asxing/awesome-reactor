...
/**
 * Exceptions are for exceptional situations. The ReactiveStreams contract specifies that exceptions
 * are terminal operations.
 *
 * <p>There are however operators available for error flow control
 */
public class Part08ErrorHandling implements BaseTestFlux {

    /**
     * After the map() operator encounters an error, it triggers the error handler in the map
     * operator, which also unsubscribes(cancels the subscription) from the stream, therefore
     * 'yellow' is not even sent downstream.
     */
    @Test
    public void errorIsTerminalOperation() {
        Flux<String> colors =
                Flux.just("green", "blue", "red", "yellow")
    ...
    /**
     * The 'onErrorReturn' operator doesn't prevent the unsubscription from the 'numbers' and
     * 'colors' stream of the map operator, but it does translate the exception for the downstream
     * operators and the final Subscriber which receives it in the 'onNext()' instead in 'onError()'
     */
    @Test
    public void onErrorReturnDoesntPreventUnsubscription() {
        Flux<Integer> numbers =
                Flux.just("1", "3", "a", "4", "5", "c")
        ...
        Flux<String> colors =
                Flux.just("green", "blue", "red", "yellow", "blue")
                        .map(
                                color -> {
                                    if ("red".equals(color)) {
                                        throw new RuntimeException("Encountered red");
    ...
    public void onErrorReturnWithFlatMap() {
        // flatMap encounters an error when it subscribes to 'red' substreams and thus unsubscribe
        // from
        // 'colors' stream and the remaining colors still are not longer emitted
        Flux<String> colors =
                Flux.just("green", "blue", "red", "white", "blue")
                        .flatMap(color -> simulateRemoteOperation(color))
                        .onErrorReturn(
                                "-blank-"); // onErrorReturn just has the effect of translating

        subscribeWithLog(colors);

        log.info("*****************");

        // bellow onErrorReturn() is applied to the flatMap substream and thus translates the
        // exception to
                        ...
                        .flatMap(
                                color ->
                                        simulateRemoteOperation(color)
                                                .onErrorReturn(
                                                        "-blank-") // onErrorReturn doesn't trigger
                                // the onError() inside flatMap so it doesn't unsubscribe from
                                // 'colors'
                                );
    ...
    /**
     * onErrorResumeWith() returns a stream instead of an exception, useful for example to invoke a
     * fallback method that returns also a stream
     */
    @Test
    ...
    /**
     * timeout operator raises exception when there are no events incoming before it's predecessor
     * in the specified time limit
     *
     * <p>retry() resubscribes in case of exception to the Observable
     */
    @Test
    public void timeoutWithRetry() {
        Flux<String> colors =
                Flux.just("red", "blue", "green", "yellow")
                        .concatMap(
                                color ->
                                        delayedByLengthEmitter(ChronoUnit.SECONDS, color)
    ...
    */
    ...
    /**
     * A more complex retry logic like implementing a backoff strategy in case of exception This can
     * be obtained with retryWhen(exceptionObservable -> Observable)
     *
     * <p>retryWhen resubscribes when an event from an Observable is emitted. It receives as
     * parameter an exception stream
     *
     * <p>we zip the exceptionsStream with a .range() stream to obtain the number of retries,
     * however we want to wait a little before retrying so in the zip function we return a delayed
     * event - .timer()
     *
     * <p>The delay also needs to be subscribed to be effected so we also need flatMap
     */
    //	@Test
    ...
    //								exceptionStream.zipWith(Flux.range(1, 3), (exc, attempts) -> {
    ...
    //                                    if (attempts < 3) {
    //                                        return Flux.
    //                                                interval(Duration.of(2 * attempts,
    // ChronoUnit.SECONDS));
    //                                    }
    //                                    return Flux.error(exc);
    //                                })
    ...
    private static final ConcurrentHashMap<String, AtomicInteger> attemptsMap =
            new ConcurrentHashMap<>();

    private Flux<String> simulateRemoteOperation(String color, int workAfterAttempts) {
        return Flux.create(
                subscriber -> {
                    AtomicInteger attemptsHolder =
                            attemptsMap.computeIfAbsent(color, (colorKey) -> new AtomicInteger(0));
